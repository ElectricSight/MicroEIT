...
class ExtractIndexes:
    ...
    def __post_init__(self):
        ...
        if any(s in self.imaging for s in ["Time", "Absolute"]):
            ...
            self.set_FD_mode()
    def read_latest_ref_idx_from_file(self):
        with open("E:/Chen/EIT/eit_application/measurements/ref_idx.txt", "r") as f:
            lines = f.readlines()
        if lines:
            return int(lines[-1])  # 返回文件中最后一行的整数值
        else:
            return None  # 如果文件为空，返回 None
    def set_ref_idx(self, idx: int):
        latest_ref_idx = self.read_latest_ref_idx_from_file()
        
        if self.ref_idx is not None and latest_ref_idx ==0:
           
           self.ref_idx = idx
        elif latest_ref_idx !=0 :
            self.ref_idx = latest_ref_idx 
...
class MeasurementDataset(
    ...
    def param_trans(self):
        self.Fmin =  eng(self.dev_setup.get_freq_min(), "Hz")
        self.Amp = eng(self.dev_setup.get_exc_amp(), "A")
    def get_latest_filename_from_log(self):
    # 打开 filename_log.txt 文件
        with open("E:/Chen/EIT/eit_application/measurements/filename_log.txt", "r") as f:
        # 读取文件内容并获取最后一行
             lines = f.readlines()
             if lines:
                latest_filename = lines[-1].strip()  # 获取最后一行并去除空格和换行符
                filename, file_extension = os.path.splitext(latest_filename)
                return filename
             else:
                return None
    def init_4_start(self, data: DataInit4Start) -> None:
        ...
        - set output dir
        """
        filename = self.get_latest_filename_from_log()
        self.dev_setup = data.dev_setup
        self.time_stamps = get_datetime_s()
        self.param_trans()
        folder = append_date_time(f'{self.time_stamps}_{filename}_{ self.Fmin }_{self.Amp}')
...
if __name__ == "__main__":
    ...
    print(
        os.path.split(
            "E:/Chen/EIT/eit_application/measurements/0olddata/old_measurements/old_measurements_json/reffish_0.1uA_1k_d100_20220301_152132"
        )
    )
